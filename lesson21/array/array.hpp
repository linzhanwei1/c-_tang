...
template <typename T> int Array<T>::length(void) { return mLength; }
...
template <typename T> T &Array<T>::operator[](int i) { return mSpace[i]; }
...
template <typename T> bool Array<T>::operator==(const Array<T> &obj) {
  bool ret = true;
...
template <typename T> bool Array<T>::operator!=(const Array &obj) {
  return !(*this == obj);
}
